...
- `auto size from detected mixels` estimates likely source-pixel cell sizes from horizontal and vertical edge profiles, then exposes the best candidate logical resolutions through the `Auto variant` slider. It also keeps a protected high-detail `native` candidate down to a 1.0px source cell, so weakly upscaled or near-original 1024px+ art can still be selected instead of disappearing behind lower-resolution mixel guesses. Turning this off keeps manual `Width` and `Height` fully in control.
- Auto detection now scores both horizontal and vertical source axes with edge alignment, local peak phase concentration, and autocorrelation support, then reports confidence, x/y ratio, and source-axis metadata in the API. It also evaluates standard detail widths such as 768 and 1024 when they fit the source aspect and detector score. The GUI keeps the visible variant list simple and shows only the candidate pixel resolutions. `Grid topology` switches between the old phase-aligned uniform grid and elastic line cuts; `Axis repair` is selectable for elastic mode and disabled for uniform mode because the old path does not use it.
